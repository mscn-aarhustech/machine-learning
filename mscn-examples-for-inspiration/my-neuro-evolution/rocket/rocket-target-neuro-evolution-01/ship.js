...
class Ship {
    constructor(brain){
		...
		//this.pos = new Vector2(200, 100 + Math.random() * (height - 200));
		this.pos = new Vector2(50 + Math.random() * (width - 100 ), 50 + Math.random() * (height - 100 ));
    ...
    //     this.pos = new Vector2(targets[0].pos.add(new Vector2(-300, -300.0)))
    //     //ship.vel = circularOrbitVelocityVector(targets[0], ship).mul(1);
    //     //this.vel = new Vector2();
    ...
    calculateFitness() {
		...
		const timeoutModifier = 10;

		let distanceToTarget = Vector2.distance(this.pos, targets[0].pos);
		
		// Base fitness
		this.fitness = this.ticksAlive * ticksModifier + distanceToTarget * distanceModifier;
