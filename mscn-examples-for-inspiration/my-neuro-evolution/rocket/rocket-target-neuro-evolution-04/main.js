...
function render() {
    ...
    for (let i = 0; i < ships.length; i++) {
        ...
        if (ship.thrust > 0.0) {
            ...
            var grd = ctx.createLinearGradient(0, 0, 0, (ship.thrust/ship.maxThrust) * 50);
            grd.addColorStop(0, "rgba(255, 192, 64, 255)");
            grd.addColorStop(1, "rgba(255, 192, 64, 0)");
            ctx.fillStyle = grd;
            ctx.fillRect(-0.5*ship.radius, ship.radius, ship.radius, ship.radius + (ship.thrust/ship.maxThrust) * 50);
            ctx.restore();
        }
    }
