...
function evaluate() {
    for (let i = ships.length - 1; i >= 0; i--) {
        if (ships[i].timeouts() || ships[i].tooFarAwayFromTarget() || ships[i].hitsTarget()) {
        //if (ships[i].tooFarAwayFromTarget() || ships[i].hitsTarget() || ships[i].timeouts() || ships[i].hitsObstacle()) {
            // The ship has died
    ...
    if (ships.length === 0) {
        ...
        for (let i = 0; i < newIndividuals.length; i++) {
            //let ship = new Ship(Network.createInstance(newIndividuals[i].genome, nnParams));
            let ship = new Ship();
...
function render() {
    ...
    for (let i = 0; i < ships.length; i++) {
        ...
        if (ship.thrust > 0.0) {
            ...
            var grd = ctx.createLinearGradient(0, 0, 0, ship.thrust * 400);
            grd.addColorStop(0, "rgba(255, 192, 64, 255)");
            grd.addColorStop(1, "rgba(255, 192, 64, 0)");
            ctx.fillStyle = grd;
            ctx.fillRect(-0.5*ship.radius, ship.radius, ship.radius, ship.radius + ship.thrust * 400);
            ctx.restore();
        }
    }
    ...
    // Render targets as a blue circle with a white border and a red dot in the center
    for (let i = 0; i < targets.length; i++) {
        let target = targets[i];
        ctx.beginPath();
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.arc(target.pos.x, target.pos.y, target.radius * 0.8, 0, 2 * Math.PI);
        ctx.fillStyle = "rgb(64, 64, 255)";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(target.pos.x, target.pos.y, target.radius * 0.2, 0, 2 * Math.PI);
        ctx.fillStyle = "rgb(255, 64, 64)";
        ctx.fill();
        //ctx.stroke();
    }

    // Render obstacles as red circles with black borders
...
// let nn1 = new Network(null, nnParams);
// console.log(nn1);

// let genome = nn1.encode();
// console.log(genome);

// let nn2 = new Network(genome, nnParams);
// console.log(nn2);
