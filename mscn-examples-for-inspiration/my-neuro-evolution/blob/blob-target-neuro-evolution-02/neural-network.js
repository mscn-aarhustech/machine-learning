...
class Neuron {
    ...
    init() {
        ...
        this.n = randomFloatBetween(0, 2.0);
        this.input = 0
        this.output = 0
    }
...
class Network {
    ...
    encode() {
        ...
        for (var i = 0; i < this.connections.length; i++) {
            ...
            //chromosome.push(this.connections[i].weight);
        }
        for (var i = 1; i < this.layers.length; i++) {
            for (var j = 0; j < this.layers[i].neurons.length; j++) {
                chromosome.push(map(this.layers[i].neurons[j].bias, -10, 10, 0, 1));
                //chromosome.push(this.layers[i].neurons[j].bias);
            }
        }
        for (var i = 1; i < this.layers.length; i++) {
            for (var j = 0; j < this.layers[i].neurons.length; j++) {
                chromosome.push(map(this.layers[i].neurons[j].n, 0, 4, 0, 1));
                //chromosome.push(this.layers[i].neurons[j].n);
            }
        }
        return chromosome;
    }
    ...
    decode(chromosome) {
        ...
        // Sort neurons by bias
        for (var i = 1; i < this.layers.length; i++) {
            this.layers[i].neurons.sort((a, b) => {
        ...
        for (var i = 0; i < this.connections.length; i++) {
            ...
            //this.connections[i].weight = chromosome[chromosomeIndex];
            chromosomeIndex++;
        }
        for (var i = 1; i < this.layers.length; i++) {
            for (var j = 0; j < this.layers[i].neurons.length; j++) {
                this.layers[i].neurons[j].bias = map(chromosome[chromosomeIndex], 0, 1, -10, 10);
                //this.layers[i].neurons[j].bias = chromosome[chromosomeIndex];
                chromosomeIndex++;
            }
        }
        for (var i = 1; i < this.layers.length; i++) {
            for (var j = 0; j < this.layers[i].neurons.length; j++) {
                this.layers[i].neurons[j].n = map(chromosome[chromosomeIndex], 0, 1, 0, 4);
                //this.layers[i].neurons[j].n = chromosome[chromosomeIndex];
                chromosomeIndex++;
            }
        }
    }
    static decode(chromosome, params) {
        let network = new Network(null, params);
        network.decode(chromosome);
        return network;
    }
}