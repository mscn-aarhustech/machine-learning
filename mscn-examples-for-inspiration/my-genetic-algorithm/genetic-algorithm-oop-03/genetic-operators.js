...
//   Takes population and number of parents as parameters
//   Return array of parent genes (not parent individuals)
// ******************************************************
...
function rouletteWheelSelection(population = [], numParents = 2) {
...
function truncationSelection(population = [], numParents = 2, matingNum = Math.floor(population.length * 0.5)) {
    let parentGenes = [];
    population.sort((a, b) => a.fitness - b.fitness);
    for (let i = 0; i < numParents; i++) {
...
function randomSelection(population = [], numParents = 2) {
...
function wholeArithmeticRecombination(parentGenes = [], numChildren = 1) {
    // Only used for genomes encoded as floating point numbers.
...
function randomizeMutation(genes = [], mutationChance = 0.01, minValue = 0, maxValue = 1) {
