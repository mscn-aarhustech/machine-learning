...
const TARGET_MAX = 100; //Math.max(...TARGET);
const TARGET_MIN = -100; //Math.min(...TARGET);

// Polynomial coefficients
// y = CF[0] * x^n + ... + CF[n-2] * x^2 + CF[n-1] * x + CF[n]
// Copy-paste below into Desmos graphing tool: https://www.desmos.com/calculator
// y\ =\ -0.023x^{4}+0.454x^{3}\ -2.759x^{2}+5.12x+2.35
const CF = [-0.023, 0.454, -2.759, 5.12, 2.35]

const FITNESS_THRESHOLD = 0.0001;
...
function main() {
    ...
    while(!found && generation < MAX_ITERATIONS) {
    //for (let i = 0; i < 20; i++) {
        ...
        let deltaFitness = Math.abs(bestFitness - prevBestFitness);

        // break the loop if we have reached the target (fitness = 0)
        if(deltaFitness != 0 && deltaFitness < FITNESS_THRESHOLD) {
            found = true;
            break;
        }
